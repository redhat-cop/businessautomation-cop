...
public class HumanTaskSteps implements En {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(HumanTaskSteps.class);
    ...
    final Function<String, Stream<TaskSummary>> retrieveTaskAsPotentialOwner = (username) -> context.getServicesClient(username, UserTaskServicesClient.class).findTasksAssignedAsPotentialOwner(username, 0, Integer.MAX_VALUE).stream();
    final Function<String, Stream<TaskSummary>> retrieveTaskOwned = (username) -> context.getServicesClient(username, UserTaskServicesClient.class).findTasksOwned(username, 0, Integer.MAX_VALUE).stream();
    ...
    final BiFunction<Optional<TaskSummary>, String, TaskSummary> claimTask = (taskSummaryOpt, username) -> {
        if (taskSummaryOpt.isPresent()) {
            ...
            context.getServicesClient(username, UserTaskServicesClient.class).claimTask(context.getReleaseId().toExternalForm(), taskSummaryOpt.get().getId(), username);
        } else {
    ...
    final BiFunction<Optional<TaskSummary>, String, TaskSummary> startTask = (taskSummaryOpt, username) -> {
        if (taskSummaryOpt.isPresent()) {
            ...
            LOGGER.debug("Task with id {} matched provided filters", taskSummaryOpt.get().getId());
            context.getServicesClient(username, UserTaskServicesClient.class).startTask(context.getReleaseId().toExternalForm(), taskSummaryOpt.get().getId(), username);
        } else {
            fail("No task found matching provided filters");
        }
        return taskSummaryOpt.get();
    ...
    final TriFunction<Optional<TaskSummary>, String, DataTable, TaskSummary> completeTask = (taskSummaryOpt, username, parameters) -> {
        if (taskSummaryOpt.isPresent()) {
            ...
            if (taskParameters.isEmpty()) {
                context.getServicesClient(username, UserTaskServicesClient.class)
                .completeTask(context.getReleaseId().toExternalForm(), taskSummaryOpt.get().getId(), username, null);
            } else {
                context.getServicesClient(username, UserTaskServicesClient.class)
                        .completeTask(context.getReleaseId().toExternalForm(), taskSummaryOpt.get().getId(), username, taskParameters);
            }
        } else {
            fail("No task found matching provided filters");
        }
